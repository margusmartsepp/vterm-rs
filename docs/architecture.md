...
```
                                       ...
                                       │                                                   │
                            ┌──────────▼─────────┐                            ┌────────────▼──────────┐
                            │   Watchdog task    │                            │   Pipe server task    │
                            │  500 ms tick       │                            │  one accept loop      │
                            │  enforces          │                            │  per-conn Reaper      │
                            └────────────────────┘                                         │
                                                                                           │ NDJSON
                                                                              ┌────────────▼──────────┐
                                                                              │  Tower pipeline       │
...
```text
Spawning ──► Ready ──► Running ──► Closing ──► Reaped
            (PTY     (initial   (any active   (child.kill,
             open,    cmd       command       child.wait,
             prompt   sent)     in flight)    pipe shut)
             seen)
```
...
```text
Connect ──► Handshake (req_id 0 Hello) ──► Active ──► Disconnect ──► Reaper drains
                                                                     all owned PTYs
```
...
```
    ...
    │             │             │             │
 ┌──┴───┐    ┌────┴───┐    ┌────┴────┐   ┌────┴────┐
 │protocol│   │ window │    │ session │   │ service │
 └──▲───┘    └────▲───┘    └────▲────┘   └────▲────┘
    │             │             │             │
    └─────────────┴──────┬──────┴─────────────┘
                         │
