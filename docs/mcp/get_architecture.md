...
## Corpus Check
- 43 files Â· ~18,784 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 248 nodes Â· 450 edges Â· 13 communities detected
- Extraction: 51% EXTRACTED Â· 49% INFERRED Â· 0% AMBIGUOUS Â· INFERRED: 222 edges (avg confidence: 0.8)
...
- `drive_claude_tui()` --calls--> `VTermClient`  [INFERRED]
  scripts\claude_orchestrator.py â†’ vterm-python\src\lib.rs
- `reality_check_claude()` --calls--> `VTermClient`  [INFERRED]
  scripts\reality_check_claude.py â†’ vterm-python\src\lib.rs
- `reality_check_claude()` --calls--> `close()`  [INFERRED]
  scripts\reality_check_claude.py â†’ vterm-python\vterm_python\server.py
- `terminal_bridge()` --calls--> `VTermClient`  [INFERRED]
  scripts\terminal_bridge.py â†’ vterm-python\src\lib.rs
- `terminal_close()` --calls--> `close()`  [INFERRED]
  tests\python_sdk\test_fastmcp.py â†’ vterm-python\vterm_python\server.py
