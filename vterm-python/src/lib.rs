...
use vterm_protocol::{BatchArgs, SkillCommand, SpawnArgs};
...
impl VTermClient {
    ...
    fn new() -> PyResult<Self> {
        ...
        let client = rt
            .block_on(async { OrchestratorClient::connect().await })
            .map_err(|e| {
                PyRuntimeError::new_err(format!("Failed to connect to orchestrator: {}", e))
            })?;
    ...
    fn spawn(
        ...
        if let Some(cb) = callback {
            ...
            let py_res = pythonize::pythonize(py, &res)
            ...
            let cb_res = match cb.call1(py, (py_res,)) {
                ...
                Err(e) => {
                    ...
                    return Err(pyo3::exceptions::PyRuntimeError::new_err(
                        "Python callback execution failed",
                    ));
                }
            };
            ...
            let json_str: String = json_module.call_method1("dumps", (cb_res,))?.extract()?;
    ...
    fn get_info(&self, py: Python<'_>, assurance: bool) -> PyResult<PyObject> {
        let res = py.allow_threads(|| {
            self.rt.block_on(async {
                self.client
                    .execute(SkillCommand::Inspect { assurance })
                    .await
    ...
    fn read(&self, py: Python<'_>, id: u32, history: bool) -> PyResult<String> {
        py.allow_threads(|| {
            self.rt.block_on(async {
                let res = self
                    .client
                    .execute(SkillCommand::ScreenRead { id, history })
                    .await
        ...
        py.allow_threads(|| {
            self.rt.block_on(async {
                let res = self
                    ...
                    .await
                    .map_err(|e| {
                        PyRuntimeError::new_err(format!("WaitUntilStable failed: {}", e))
                    })?;
                Ok(res.content.unwrap_or_default())
            })
        })
    ...
    fn screen_diff(&self, py: Python<'_>, id: u32) -> PyResult<String> {
        py.allow_threads(|| {
            self.rt.block_on(async {
                let res = self
                    .client
                    .execute(SkillCommand::ScreenDiff { id })
                    .await
    ...
    fn batch(
        ...
        let res = py.allow_threads(|| {
            self.rt.block_on(async {
                self.client
                    .execute(SkillCommand::Batch(batch_args))
                    .await
