...
impl TerminalServer {
    #[tool(name = "get_info", description = "Returns version and resource telemetry for the orchestrator.")]
    async fn get_info(&self) -> String {
        match self.client.execute(SkillCommand::Inspect { assurance: true }).await {
            Ok(res) => serde_json::to_string_pretty(&res).unwrap_or_else(|_| "{}".into()),
            Err(e) => format!("Error: {e}"),
        }
    }

    #[tool(name = "list_terminals", description = "Returns a list of all active terminal IDs and their titles.")]
    ...
    #[tool(name = "spawn", description = "Spawns a new terminal session. Returns the terminal ID and any extracted data.")]
    async fn spawn(&self, args: Parameters<McpSpawnArgs>) -> String {
        ...
        match self.client.execute(SkillCommand::Spawn(req)).await {
            Ok(res) => serde_json::to_string_pretty(&res).unwrap_or_else(|_| "Spawned successfully.".into()),
            Err(e) => format!("Error: {e}"),
        }
    }

    #[tool(name = "write", description = "Writes keystrokes (like <Enter> or <Up>) or text to a terminal.")]
    ...
    #[tool(name = "read", description = "Reads the current contents of the terminal screen. Set history=true for full scrollback.")]
    async fn read(&self, args: Parameters<McpReadArgs>) -> String {
        match self.client.execute(SkillCommand::ScreenRead { id: args.0.id, history: args.0.history }).await {
            Ok(res) => res.content.unwrap_or_default(),
            Err(e) => format!("Error: {e}"),
        }
    }

    #[tool(name = "wait_until_stable", description = "Blocks until the screen buffer remains unchanged for a specified duration.")]
    async fn wait_until_stable(&self, args: Parameters<McpWaitStableArgs>) -> String {
        match self.client.execute(SkillCommand::WaitUntilStable { 
            id: args.0.id, 
    ...
    #[tool(name = "screen_diff", description = "Returns only the visual delta since the last observation for this terminal.")]
    async fn screen_diff(&self, args: Parameters<McpIdArgs>) -> String {
        match self.client.execute(SkillCommand::ScreenDiff { id: args.0.id }).await {
            Ok(res) => res.content.unwrap_or_default(),
            Err(e) => format!("Error: {e}"),
        }
    }

    #[tool(name = "extract", description = "Extracts structured data from terminal history using a regex with named groups.")]
    async fn extract(&self, args: Parameters<McpExtractArgs>) -> String {
        match self.client.execute(SkillCommand::Extract { 
            id: args.0.id, 
    ...
    #[tool(name = "wait_until", description = "Waits until a regex pattern appears on the screen, streaming live output via progress notifications.")]
    async fn wait_until(
        &self, 
        ...
        let progress_token_str = progress_token.as_ref().map(|t| {
            serde_json::to_string(t).unwrap_or_default().replace("\"", "")
        });
        ...
        let event_handle = tokio::spawn(async move {
            if let Some(token) = token_clone {
                while let Ok(event) = events.recv().await {
                    match event {
                        vterm_rs::protocol::Event::Progress { token: ev_token, percentage, msg, .. } => {
                            let token_str = serde_json::to_string(&token).unwrap_or_default().replace("\"", "");
                            if ev_token == Some(token_str) {
                                let _ = peer_clone.notify_progress(rmcp::model::ProgressNotificationParam {
        ...
        let res = self.client.execute_full(
    ...
    #[tool(name = "batch", description = "Executes a batch of commands. Supports progress notifications.")]
    async fn batch(
        &self, 
        ...
        let res = self.client.execute_full(SkillCommand::Batch(batch_args), progress_token_str).await;
        event_handle.abort();

        match res {
            Ok(res) => serde_json::to_string_pretty(&res).unwrap_or_else(|_| "Batch completed.".into()),
            Err(e) => format!("Error: {e}"),
        }
    }

    #[tool(name = "get_process_state", description = "Returns the current process state (running: bool, exit_code: int?).")]
    async fn get_process_state(&self, args: Parameters<McpIdArgs>) -> String {
        match self.client.execute(SkillCommand::GetProcessState { id: args.0.id }).await {
            Ok(res) => serde_json::to_string_pretty(&res).unwrap_or_else(|_| "{}".into()),
            Err(e) => format!("Error: {e}"),
        }
    }

    #[tool(name = "get_architecture", description = "Returns the architectural knowledge graph report for this codebase.")]
    async fn get_architecture(&self) -> String {
        match tokio::fs::read_to_string("graphify-out/GRAPH_REPORT.md").await {
            ...
            Err(_) => match tokio::fs::read_to_string("GRAPH_REPORT.md").await {
                Ok(content) => content,
                Err(e) => format!("Error reading architecture report: {e}. Ensure graphify has been run."),
            }
        }
    }

    #[tool(name = "rust_eval", description = "Evaluates a Rust code snippet using evcxr. Returns the output or error.")]
    async fn rust_eval(&self, args: Parameters<McpEvalArgs>) -> String {
        ...
        let mut child = match Command::new("evcxr")
            ...
            .spawn() {
                Ok(c) => c,
                Err(e) => return format!("Error: Failed to spawn evcxr. Ensure it is installed and in your PATH. ({e})"),
            };
