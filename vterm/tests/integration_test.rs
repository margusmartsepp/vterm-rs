...
use vterm_rs::{BatchArgs, CommandResult, Response, SkillCommand, SkillRequest, SpawnArgs, Status};
...
async fn test_supreme_orchestration() -> Result<()> {
    ...
    let playbook1 = SkillCommand::Batch(BatchArgs {
        commands: vec![
            ...
            SkillCommand::WaitUntil {
                id: term_id,
                pattern: "Ping statistics".into(),
                timeout_ms: 15000,
            },
            SkillCommand::ScreenRead {
                id: term_id,
                history: false,
            },
        ],
    ...
    let content = screen_read
        .content
        .as_ref()
        .expect("ScreenRead should have content");
